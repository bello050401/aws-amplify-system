...
test('only current human blocker foreground; old pending approval deferred and terminal archived',()=>{
 ...
 const rows=planTodos([todo('1','本番データ大量削除'),todo('2','本番DB破壊的移行','old'),todo('3','git commit','done'),todo('4','テストを再実行')],tasks,{currentTaskId:'t'});
...
test('approval categories do not turn ordinary git into human work; sensitive actions stay human',()=>{
 ...
 for(const title of ['本番データ大量更新','破壊的DB migration','IAM権限拡大','S3広範ポリシー変更','Cognito削除','実EC出品','ZAICO本番データ変更','課金操作','MFA入力','OAuth認証','ログイン'])assert.equal(requestOwner(todo('x',title)),'human',title);
 for(const title of ['production deploy','本番へ公開','mainブランチ反映'])assert.equal(requestOwner(todo('x',title)),'ai',title);
...
test('paused historical items revive when related task runs; no arbitrary five-item cap',()=>{
 const list=Array.from({length:7},(_,i)=>todo(''+i,'本番データ大量削除 '+i));assert.ok(planTodos(list,[{id:'t',state:'paused'}]).every(t=>t.triage.bucket==='archive'));assert.equal(planTodos(list,[{id:'t',state:'running'}]).filter(t=>t.triage.bucket==='now').length,7);
});
...
test('completed focus never resurrects an archived human approval',()=>{assert.equal(planTodos([todo('1','本番データ大量削除')],[{id:'t',state:'completed'}],{currentTaskId:'t'})[0].triage.bucket,'archive')});
...
test('verified AI completion closes only assigned AI requests and keeps task status',async()=>{const h=await buildHarness();try{const task=h.repo.createTask({title:'task',instruction:'test',repoPath:h.config.repoPath}).task;h.repo.createTodo({title:'再テスト',waitingTaskIds:[task.id]});h.repo.createTodo({title:'本番データ大量削除',waitingTaskIds:[task.id]});h.todoManager.triage.markRunning(task.id);h.todoManager.triage.completeVerified(task.id);assert.equal(h.repo.listTodos({status:'open'}).length,2);h.store.run("UPDATE tasks SET state='completed' WHERE id=?",[task.id]);h.todoManager.triage.completeVerified(task.id);assert.equal(h.repo.listTodos({status:'open'})[0].title,'本番データ大量削除');assert.equal(h.repo.getTask(task.id).state,'completed')}finally{h.cleanup()}});

test('one completion resolves an exact duplicate group without deleting its history',async()=>{const h=await buildHarness();try{const a=h.repo.createTask({title:'a',instruction:'test',repoPath:h.config.repoPath}).task;const b=h.repo.createTask({title:'b',instruction:'test',repoPath:h.config.repoPath}).task;const first=h.repo.createTodo({title:'MFA入力',category:'mfa',waitingTaskIds:[a.id],dedupeKey:'first'}).todo;h.repo.createTodo({title:'MFA入力',category:'mfa',waitingTaskIds:[b.id],dedupeKey:'second'});h.todoManager.completeGroup(first.id,{actor:'user',answer:'done'});assert.equal(h.repo.listTodos().length,2);assert.equal(h.repo.listTodos({status:'completed'}).length,2);}finally{h.cleanup()}});
