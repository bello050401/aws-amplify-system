  ...
  input_schema: {
    ...
    properties: {
      ...
      introduction: {
        type: "string",
        description:
          // 2026-09-09 追加指示: 傷・錆・汚れ等のコンディション情報が
          // 「◎商品のご紹介」に混入する不具合への対応。寸法だけを禁止
          // していた既存文言に、状態説明の禁止を明示で追加する
          // (conditionSectionへ書くべき情報なので、ここへ来ると
          // §19のfixedSections除外と矛盾した内容が本文に混在する)。
          "「◎商品のご紹介」の本文。家具の魅力・ブランド・デザイン・素材感・用途・雰囲気を自然な文章で書く。寸法の羅列は書かない。傷・汚れ・錆・スレなどのコンディション(状態)の説明も書かない — それはconditionSectionの役割。",
      },
...
export function buildProductPageSystemPrompt(profile: BelloStyleProfile | null): string {
  ...
  lines.push(
    ...
    // 作られた国は、いずれも別の情報。
    "- ブランドが生まれた国・素材の産地と、この商品個体の製造国は別の情報。製造国が事実情報として明示されていない限り、「〜製」「原産国は〜」「made in 〜」のように製造国を書かない。ブランド名に国名が含まれている、ブランドがある国で有名、または素材の産地が分かっているからといって、それを製造国として書かない。製造国が分からない場合は断定せず、書かないか担当者へ確認する。",
    // 2026-09-12 追加指示: 「イタリア製レザーを使用」「脚はイタリア製」
    // のように部材・交換部品・素材だけの製造国を、完成品(個体)全体の
    // 製造国であるかのように書いた/読み取れる不具合への対応。
    "- 脚・部材・交換部品・素材(レザー・生地等)の製造国・産地と、完成品(個体)全体の製造国は別の情報。部材や素材の産地が分かっていても、それを完成品全体の製造国として書かない。完成品そのものの製造国が事実として明示されている場合だけ、完成品の製造国として書く。",
    // 2026-09-11 追加指示: 照明をデザイナーズ「家具」と呼んだ不具合への
    // 対応。カテゴリはfactsBlockで既に渡しているが、それを無視して一般的な
    // 呼び方(「家具」)へ寄せてしまうことがあるため明示で禁止する。
    "- 事実情報の「カテゴリ」欄に従って商品を呼ぶ。カテゴリと異なる一般名称(例: 照明を「家具」と呼ぶ)を使わない。",
    "- この指示文自体を出力に含めない。",
...
export function buildProductPageUserPrompt(input: {
  ...
  if (fixed.length > 0) {
    blocks.push(
      ...
      // 2026-09-09 追加指示: 寸法・配送方法等と同列にコンディションも
      // 明示する。「◎コンディション」はルールベースで別途確定生成する
      // (このプロンプトのconditionSectionは本番経路では使われない)ため、
      // 「商品のご紹介」に傷・状態の話が滲み出るのを防ぐ。
      "「商品のご紹介」の中でも、寸法・配送方法・送料・返品条件・お取り置き条件・コンディション(傷・汚れ・錆・スレなどの状態)には触れないでください。",
      "==== ここまで ====",
    );
  }
