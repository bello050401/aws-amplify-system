 ...
 * MercariShopsClient.tsから移植。ロジックは無変更(タイムアウト/
 * リトライ/エラー分類の考え方はそのブランチの実装をそのまま踏襲) —
 * 変更したのはトークン取得元(元はPrisma越しのAppSetting、こちらは
 * lib/listing/mercari/tokenAccess.tsのAWS Secrets Manager/環境変数)
 * だけで、それも呼び出し元がconstructor引数として注入するだけなので
 * このファイル自体は無変更で済んでいる。
 */
 ...
 * - GraphQLエラー/HTTPエラーは握り潰さず MercariApiError として投げる。
 * - 429/5xx は指数バックオフで最大 maxRetries 回まで再試行。ミューテーションは二重実行を避けるため既定でリトライしない。
 */
export class MercariShopsClient {
  ...
  private async singleRequest<TData, TVariables>(query: string, variables: TVariables): Promise<TData> {
    ...
    try {
      ...
      throw new RetryableHttpError(err instanceof Error ? err : new Error("Network error calling Mercari Shops API"));
    } finally {
      clearTimeout(timer);
    }

    if (response.status === 429 || response.status >= 500) {
      throw new RetryableHttpError(new Error(`Mercari Shops API returned HTTP ${response.status}`));
    }

    if (!response.ok) {
      const bodyText = await safeReadText(response);
      throw new MercariApiError(`Mercari Shops API request failed with HTTP ${response.status}`, [{ message: bodyText || response.statusText }]);
    }
    ...
    if (json.errors && json.errors.length > 0) {
      throw new MercariApiError(json.errors.map((e) => e.message).join("; "), json.errors, requestId);
    }
    if (!json.data) {
