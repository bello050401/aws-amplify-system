...
const AMBIGUOUS_PATTERNS: { re: RegExp; note: string }[] = [
  ...
  // 指示語は「連体修飾(そちらの商品)」なら直後に指す対象が書かれて
  // いるので曖昧ではない。裸の代名詞として使われている場合だけ拾う。
  //
  // 以前は /それ|あれ|そちら|あちら|例の/ という単純な部分一致だった
  // ため、「そちらの商品は…」のようなごく普通の丁寧文でも必ず注意が
  // 出ていた。AI返信案を整えるたびに毎回出るので、利用者からは
  // 「敬語に整えるとエラーになる」ように見えていた。
  {
    re: /(?:それ|あれ|そちら|あちら)(?=[はをがにでも、。\s]|$)|例の/,
    note: "指示語が何を指すか、文章だけでは分かりません。",
  },
...
export function buildKeigoSystemPrompt(): string {
  return [
    ...
    "",
    "【改行】",
    "- 下書きの段落の切れ目(空行)をそのまま残してください。1つの段落へまとめないでください。",
    "- 挨拶・本題・確認事項・締めは、それぞれ別の段落にしてください。",
    "- 1段落が3文を超えたら、意味の切れ目で改行してください。",
  ].join("\n");
}
