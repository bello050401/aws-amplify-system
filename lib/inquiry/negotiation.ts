...
const EXPLICIT_PATTERNS: { re: RegExp; label: string }[] = [
  ...
  // 実機で取りこぼした形(2026-09-03)。
  //
  //     https://…/items/156144635
  //     3万円まで下げられますか？
  //
  // 「値下げ」「値引き」「安く」「交渉」のどれも現れず、既存の表では
  // 交渉として認識できなかった。金額は読めているのに交渉と判定されない
  // ため、値下げ判断のルートへ一度も入らないまま一般回答になっていた。
  { re: /下げ(?:られ|て|ていただ|ませ|ま)/, label: "「下げられますか」" },
  { re: /下が(?:り|ら)(?:ませ|ない|ます)/, label: "「下がりませんか」" },
  { re: /お?値下/, label: "「お値下げ」" },
];
