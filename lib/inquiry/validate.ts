...
export function validateReplyDraft(params: {
  ...
  /**
   * 既に分かっているご希望のお届け日(2026-09-11 追加)。
   *
   * conversationContext.ts の PendingQuestion は「ご希望のお届け日」を
   * 配送先都道府県と全く同じ仕組み(質問検出→回答検出→会話文脈へ保存)で
   * 追跡しているのに、生成後の検査(このファイル)には都道府県の再質問
   * だけが弾かれ、お届け日の再質問を弾く対がなかった —— 一度答えた
   * お届け日をもう一度尋ねる返信が出ても、検査を通り抜けてしまう。
   * 都道府県と同じ理由(§23 既に分かっていることを尋ねない)で、同じ形の
   * ガードを足す。
   */
  knownDeliveryDate?: string | null;
  /**
  ...
  // ── 既に分かっているお届け日を尋ねていないか(上と同じ理由) ────
  //
  // 2026-09-11 QA是正: 当初の実装は「教え/お伺い/ご指定/ご連絡/お知らせ」
  // という**語幹**の有無だけを見ていたため、「ご希望のお届け日は既に
  // お知らせいただいております。」(受領確認)にも一致してしまった ——
  // 「お知らせ」という語幹自体は、依頼(お知らせください)にも報告
  // (お知らせいただいております)にも現れる。依頼の語尾(ください /
  // いただけますか 等)が続く場合だけを「尋ねている」とみなす —— 「教えて
  // いただいた」「いただきました」のような受領報告は、語幹の直後が
  // 「いただけ」(依頼の可能形)ではなく「いただい」(過去形)なので、
  // この語尾要求だけで区別できる。
  if (params.knownDeliveryDate) {
    const DELIVERY_DATE_WORD = "(?:お届け日|配達日|配送日|納品日)";
    const REQUEST_VERB = "(?:教え|ご指定|ご連絡|お知らせ|ご教示)";
    // 「いただけ」は必ず「ます」を伴わせる —— 「教えていただけない場合も」
    // のような**否定**の言い回しまで、「いただけ」の有無だけで拾わないため。
    const REQUEST_TAIL =
      "(?:て)?(?:いただけます(?:でしょうか|か)?|いただきたく(?:存じます)?|くださいますよう|ください|下さい|お願い(?:いたします|します))";
    const asksDeliveryDateDirect = new RegExp(`${DELIVERY_DATE_WORD}[^。\\n]{0,15}${REQUEST_VERB}${REQUEST_TAIL}`).test(
      output,
    );
    // 「お伺い」は活用が異なる(お伺いします/お伺いできますでしょうか等)ので
    // 別に見る。
    const asksDeliveryDateUkagai = new RegExp(
      `${DELIVERY_DATE_WORD}[^。\\n]{0,15}お伺い(?:し|いたし)(?:ます(?:でしょうか|か)?|て(?:よろしい)?(?:でしょうか|ですか)?)`,
    ).test(output);
    // 既知の日付そのものを尋ね直すのではなく、日程調整のために必要な
    // 「別の/他の」候補日を尋ねる場合は、既に分かっている事実を尋ねて
    // いるわけではないので弾かない(発送できる根拠の無い約束は、この
    // ガードとは別にFABRICATED系の検査が引き続き扱う)。
    const asksAlternativeDate = /(?:別|他)(?:の|日程)?[^。\n]{0,10}(?:お届け日|配達日|配送日|納品日|ご希望日|日程)/.test(
      output,
    );
    if ((asksDeliveryDateDirect || asksDeliveryDateUkagai) && !asksAlternativeDate) {
      violations.push({
        code: "ASKS_KNOWN_FACT",
        detail:
          `ご希望のお届け日は既に「${params.knownDeliveryDate}」と分かっています。お届け日を尋ねてはいけません。` +
          "分かっている前提で書き、確定していないことは「確認のうえ改めてご案内いたします」としてください。",
      });
    }
  }
