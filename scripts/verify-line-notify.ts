...
function testReplyMessage() {
  ...
  // 2026-09-03 追加指示§27: 見出しを付けない。担当者はこの2通目を
  // **そのままコピーして顧客へ送る**ので、見出しが残ると顧客へ届く。
  // 1通目との区別は、1通目の末尾の【次のメッセージで返信提案します】が担う。
  assertTrue(!r.includes("【返信提案】"), "2通目: 見出し【返信提案】を付けない");
  assertTrue(r.startsWith("お問い合わせありがとうございます。"), "2通目: 顧客へ送る本文だけ");
