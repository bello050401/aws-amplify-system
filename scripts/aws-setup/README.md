...
6. **staging Appのビルドを実際に開始・監視・診断したい場合(通常はここから開始してよい)** → **`7-fix-staging-iam-role.ps1`**(書き込みあり) — 元はIAM Role専用の修正スクリプトだったが、現在は次の一連の流れを1本にまとめたstagingデプロイ検証・実行スクリプトとして整理されている: (a) staging専用ロール`BelloAmplifyStagingBackendDeploymentRole`の作成・更新(既に成功済みなので冪等、`skip`可能)、(b) ビルド開始前のpreflight(account/region/App ID安全確認・branch安全確認・platform=WEB_COMPUTE確認・framework確認・staging role ARN確認・`bello/zaico-api-token`のSecret存在確認・CloudFormation stack状態の一覧、問題があればビルドを開始せず中断)、(c) RELEASE build開始・自動ポーリング、(d) 失敗時は読み取り可能なUTF-8ログを一時ファイルへ保存しつつ画面へ出力し、"first meaningful failure"(最初の実質的な失敗行、末尾の汎用的な"Build failed"だけに頼らない)を自動抽出、IAM反映待ちパターンのみ自動リトライ、(e) 成功時は`/`・`/inventory`・`/inventory/login`のHTTP応答を確認。既存の本番Appやロールは読み取り専用でしか参照しない。詳細は`docs/aws-test-environment.md`§9・§10参照。
