  ...
  return (
    <div className="max-w-2xl">
      ...
      <details className="mb-3 text-[11px] text-gray-400">
        ...
        <p className="mt-1">
          安全条件の判定・下限価格でのブロック・実行回数上限・監査ログ（EC出品画面の「価格変更履歴」で確認可能）までは実際に動作します。下限価格に到達したときの動作（そのまま維持／出品を停止／手動確認を促す）も実際に適用され、選んだ内容に応じて出品状態が変わります。ただし、Mercari
          Shopsへ実際の価格変更を送信するAPI呼び出し自体は、公式のGraphQL仕様がこの開発環境から確認できていないため未実装です。「今すぐ価格チェックを実行」は判定結果の記録のみを行い、実際の価格は変更しません。再出品も同じ理由で未実装で、選んでも価格・状態とも変更されません（記録だけが残ります）。
        </p>
      </details>
