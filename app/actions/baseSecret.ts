...
import { BaseNotConnectedError, BaseTokenExchangeError, BaseTokenStorageError, disconnectBase } from "@/lib/base/oauth";
...
export async function testBaseConnectionAction(): Promise<BaseConnectionTestResult> {
  try {
    ...
    if (err instanceof BaseTokenExchangeError) {
      // アクセストークンの期限切れ後、リフレッシュに失敗した場合はここへ来る。
      // 「予期しないエラー」に畳むと、再連携すればよいことが伝わらない。
      return { success: false, message: err.message, retryable: err.code === null };
    }
