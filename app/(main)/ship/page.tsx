...
/** 出庫画面(指示書 §18)。入庫画面と同じ検索/スキャン導線を再利用。在庫不足はエラー表示。 */
export default function ShipPage() {
  ...
  async function confirmShip() {
    ...
    setError(null);
    try {
      const updated = await getInventoryService().shipStock(selected.id, quantity, user?.email ?? "unknown");
      setMessage(`「${selected.name}」を${quantity}${selected.unit}出庫しました(現在庫: ${formatQuantity(updated.quantity)})`);
      setSelected(null);
      setQuantity(1);
      setKeyword("");
      setCandidates([]);
  ...
  return (
    <div className="pb-24">
      <MobileHeader
        title="出庫"
        right={
          <Link href="/ship/history" className="tap-target text-xs font-semibold text-bello-700">
            履歴
          </Link>
        }
      />
          ...
          <>
            <div className="flex gap-2">
              ...
              <button
                ...
                className="tap-target flex items-center gap-1 rounded-full bg-accent-500 px-4 text-sm font-semibold text-white"
              >
              ...
              {candidates.map((c) => (
                ...
                >
                  ...
                  <span className="text-xs text-bello-400">
                    出庫可能 {formatQuantity(c.freeQuantity)}
                    {c.unit}
                  </span>
                </button>
              ))}
        ...
        {selected && (
          <div className="space-y-4 rounded-2xl bg-white p-4 shadow-card">
            <div>
              ...
              <p className="text-xs text-bello-400">
                出庫可能: {formatQuantity(selected.freeQuantity)}
                {selected.unit}
              </p>
            </div>
            <NumberInput
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={confirmShip}
                disabled={busy}
                className="tap-target flex flex-1 items-center justify-center gap-2 rounded-full bg-accent-500 py-3 text-sm font-bold text-white disabled:opacity-60"
              >
                {busy && <InlineSpinner />}
                出庫確定
              </button>
