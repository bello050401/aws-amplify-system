...
### QA-o: 価格変更履歴が書かれるだけで読まれていなかった — **修正済み**

26個のモデルすべてについて「書き込みはあるが読み出しが1箇所も無い」
ものを機械的に探したところ、該当は`PriceHistory`の1件だけだった。

`PriceHistory`は値下げ判定のたびに「変更前/変更後の価格・理由・
適用ルール・実行者・外部送信結果・日時」を記録している。にもかかわらず
どこからも読まれておらず、設定画面は
**「監査ログ（商品詳細画面から確認可能）」と案内していた**。実際に
画面へ出ていたのは`lastAutomationResult`(直近1回の判定を要約した
文字列、毎回上書き)だけで、履歴そのものは見られなかった。

EC出品画面に「価格変更履歴」を追加(新しい順、既存の
`channelListingId`インデックス経由でScanなし)。履歴の読み込みに失敗
しても値下げ設定自体は使えるようにしてある。設定画面の案内先も実態へ修正。

### QA-p: 「一覧を開くだけでZAICO同期が走らない」の再確認 — **AWS_VERIFIED**

利用者がP0とした項目。実機で在庫一覧を3回、商品詳細を1回開いた結果:

| 確認項目 | 結果 |
|---|---|
| ブラウザからZAICOへの通信 | **0件** |
| 操作前後の在庫件数 | **1,000件 → 1,000件（不変）** |
| ZAICO IDの重複 | **0件**（1,000件が1,000種すべて一意） |
| SKUの重複 | **0件** |
