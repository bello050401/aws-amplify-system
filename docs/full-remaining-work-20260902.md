...
前回の調査で「集計対象が母集団の85%なので絞り込みでは14%しか減らない」
ことが実測で分かっていた項目。効くのは集計方式そのものの変更なので、
月次集計の read model を入れた。

**加算ではなく作り直し**にしてある。加算はイベントを1つ取りこぼしただけで
静かにズレ、しかもズレたことに気づけない。対象月のレコードから毎回
作り直すので、二重計上も取消の取りこぼしも起きない。

Golden Test を先に作った(`npm run verify:sales-aggregate`)。Stagingの
実在庫5,313件で、**全46ヶ月**について 件数/売上/購入/送料/原価/粗利/
原価率/平均単価が現行ロジックと完全一致することを確認(「当月・前月・
任意月」だけでなく全部見る)。1円の変化でも drift として検出でき、
その月にだけ出ることも固定した。

```
現行: 在庫全件を読む         5,313件 / 1,546.6KB
新方式: 当月1件 + 推移12件       13件 /     1.6KB
```

商品一覧だけは明細そのものなので集計に持てない。ここは
`listInventoryBySaleMonth` でその月のぶんだけ取る。ただし DynamoDB は
フィルタ適用前に行を読むので、**往復回数は減らない** —— 減るのは転送量と
メモリ。往復も減らすには saleEndDate の年月を持つ列とGSIが要り、
既存4,511件への遡及書き込みを伴うので行っていない。

集計が無い月は黙って0にせず、その場で計算する。「集計がまだ無いので
0円です」は最悪の失敗の仕方(売上が消えたように見える)。画面には
「集計は○○時点」または「その場で計算しています(数値は同じです)」と出す。

### 設定 ＞ 商品説明文

4領域(文体プロファイル / 改善指示 / テスト生成 / 変更履歴)を追加した。

**「確からしさ 0.996」を説明可能にした。** コードを追った結果、これは
実際に計算されている値だった:

```
confidence = min(1, 分析件数 ÷ 200) × (紹介文を抽出できた件数 ÷ 分析件数)
           = min(1, 267÷200) × (266÷267) = 1 × 0.996 = 0.996
```

つまり分析件数が200件を超えた時点で前半は1で頭打ちになり、実質的には
**紹介文の抽出成功率と同じ値**。文章の良し悪しや生成結果の正しさを
表す値ではない。画面の「詳細」に、この式と分解した数値と意味をそのまま出す。

改善指示(人が書いた要望)と Style Profile(機械が数えた結果)は別モデルに
分けた。片方を更新したときにもう片方が黙って消えないようにするため。
生成時の優先順位は

```
Inventoryの確定事実 > BELLO改善指示 > Style Profile > 類似BASE商品 > 一般的なAI表現
```

で、プロンプト上も事実ブロックとは別の位置に置き「これは事実ではない。
確定事実と矛盾する指示があれば確定事実を優先すること」を明示する。
**改善指示で事実を改変することはできない** —— factSafety と紹介文の
寸法検査は改善指示より後段にあり、指示に何が書かれていても効く。

Before/After は候補を**保存しなくても**比較できる(保存しただけで全体の
生成挙動が変わることを避けるため)。復元は過去の版を書き換えず、その内容で
新しい版を作って現在の設定にする(履歴が枝分かれしないように)。
...
| 売上集計テーブルへの初回書き込み | **完了** | デプロイ(job 156)後に `npm run rebuild:sales-aggregate -- --apply` を実行。46ヶ月を書き込み、書き込み後の再検査も drift なしで一致 |
